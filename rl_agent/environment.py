...
【重要声明：本环境为 *模拟环境*】
================================================================================
用户的情绪状态转移与显式反馈均由 ``numpy`` 随机数模拟，**不包含任何真实
用户数据**。设计目的：

    1. 在真实多模态情感识别模块 (``emotion_recognition/``) 接入之前，
       独立验证 DQN 训练流程的正确性与策略收敛性；
    2. 为论文 3.3 节的「动态反馈系统」提供一个可控、可复现的试验平台。

真实部署时，应将本环境的观测（用户情绪 one-hot）替换为情感识别模型的
输出概率，将模拟用户反馈替换为真实交互数据。切勿将模拟结果当作真实用户
实验数据写入论文或报告。
